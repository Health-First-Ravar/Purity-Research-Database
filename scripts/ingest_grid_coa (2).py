...
These research COAs (Laboratorio de Quimica e Bioatividade de Alimentos / Nucleo de
Pesquisa em Cafe Prof. Luiz Carlos Trugo, UFRJ) list many coffees in a grid: a
summary table (water/color/caffeine/trigonelline) paired with a CGA-speciation
table, optionally split into green and roasted sections, sometimes with a
per-serving table. Column sets vary (lactones present only for roasted, units in
the header or in a separate row), so this parser keys on HEADER TEXT, not position,
and pairs summary+CGA tables by row order within a section.

Input: the markdown-table text produced by the Drive "read_file_content" export
(or any equivalent). Each table is header row + data rows, one row per line,
cells delimited by '|'. Run:

    python3 ingest_grid_coa.py <doc.md> <report_date YYYY-MM-DD> [--matrix green|roasted] --out DIR
