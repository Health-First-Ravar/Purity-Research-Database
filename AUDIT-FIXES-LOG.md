...
- **`3629638-0 "LIVER VITALITY"` carries `blend = PROTECT`.** If that is
  co-branded or private-label rather than Purity PROTECT, the blend column is
  wrong at source and would put it in front of CS.
- **`BRN-49871859-0`** has `blend = BALANCE` but no sample name at all.
- **One name conflict still unresolved**:
  `COA-31-Oct-25-Bette-Buna-49872102-0.pdf`, 143,950 B (base) vs 81,212 B
  (`Processed/`), different hashes. Base copy taken; the other never ingested.
- **`Purity Results - Caffeine.doc`** — legacy `.doc`, unfetchable. Convert to
  `.docx` in Drive and it comes through.
- **Sacred Cups** appears both as genuine COAs (5 rows) and in the non-COA
  filename blocklist. Reconcile.
- **`ingest.py` deletes** `Processed/*.json` each run via
  `clean-stale-processed.py`. Derived artefacts only, git-tracked, never the DB.
- **`extract_docx` is weak** — recovers analyte tables but not headers.
- **Port 3000 is used by another project** (`arbiter-v4/hub-next`). Use
  `PORT=3100` for this app; a stale server there caused 404s that looked like
  routing bugs.
